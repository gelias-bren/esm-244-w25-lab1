...
What is the spatial distribution of species?
...
Which street/zip codes have more tree density?
...
sfo_trees_df |>
  ...
  labs(y='tree count', x='legal_status')+
  theme_minimal()+
...
select legal status, lat, lon, and planting date
filter by genus: oak and pine trees. 
summarize the counts of oak and pine in separate df
...
oak_pine_df <- sfo_trees_df |>
  filter(str_detect(species, 'Quercus') | str_detect(species, 'Pinus')) |>
  select(species, legal_status, plant_date, latitude, longitude) |>
  mutate(type=ifelse(str_detect(species, 'Quercus'), 'oak', 'pine'))
...
# input into t-test
shapiro.test(oak_pine_year_df$plant_year)
